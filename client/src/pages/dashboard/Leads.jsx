...
export default function Leads() {
  ...
  const handleConvertClick = async (lead) => {
    ...
    try {
      // Fetch ALL available deals for selection
      const response = await axios.get(`/deals`);
      const allDeals = response.data?.data ?? response.data ?? [];
      // Show all deals, not just ones associated with this lead
      setAvailableDeals(allDeals);
      setSelectedDealId(allDeals.length > 0 ? allDeals[0]._id : "");
      setShowConvertModal(true);
    } catch (error) {
      console.error("Error fetching deals:", error);
      toast.error("Failed to load deals for conversion");
    }
  };

  const handleConvert = async () => {
    if (!selectedLeadForConversion || !selectedDealId) {
      toast.error("Please select a deal for conversion");
      return;
    }
    try {
      await dispatch(convertLead({ id: selectedLeadForConversion._id, dealId: selectedDealId })).unwrap();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/40 p-4 lg:p-8">
      ...
      {/* Convert Lead Modal */}
      {showConvertModal && selectedLeadForConversion && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">Convert {selectedLeadForConversion.name} to a customer</p>
                </div>
              </div>
            </div>
            <div className="p-6 space-y-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-3">
                  Select Deal for Conversion <span className="text-red-500">*</span>
                </label>
                {availableDeals.length > 0 ? (
                          ...
                          <div className="flex-1">
                            ...
                            {deal.customers && deal.customers.length > 0 && (
                              <div className="flex items-center gap-1 mt-2 text-sm text-gray-600">
                                ...
                                <span>
                                  Customer{deal.customers.length > 1 ? 's' : ''}: {deal.customers.map(c => typeof c === 'object' ? c.name || c.email || c._id : c || 'Unknown').join(', ')}
                                </span>
                              </div>
                            )}
                          </div>
                  ...
                  <div className="text-center py-8 text-gray-500 border-2 border-dashed border-gray-300 rounded-xl">
                    ...
                    <p className="font-medium">No deals available</p>
                    <p className="text-sm mt-1">You need to create a deal first before converting this lead.</p>
                    <button
                      onClick={() => {
                        setShowConvertModal(false);
                        setSelectedLeadForConversion(null);
                        setSelectedDealId("");
                        // Navigate to deals page or open create deal modal
                        window.location.href = "/dashboard/deals";
                      }}
                      className="mt-4 inline-block px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700 transition-colors"
                    >
                      Create New Deal
                    </button>
                  </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  <FiZap className="text-blue-600 mt-0.5" />
                  <div>
                    <h4 className="text-sm font-medium text-blue-900">Conversion Process</h4>
                    <p className="text-sm text-blue-700 mt-1">
                      This will create a new customer from the lead and associate the selected deal with the customer.
                      The lead will be removed after successful conversion.
                    </p>
                  </div>
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-3 bg-gradient-to-r from-emerald-600 to-green-600 text-white rounded-xl hover:from-emerald-700 hover:to-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium shadow-lg"
                >
                  Convert to Customer
                </button>
              </div>
